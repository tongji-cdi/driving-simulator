...
namespace NWH.VehiclePhysics2.Powertrain.Wheel
{
    ...
    {
        /// <summary>
        ///     Ackerman configuration for vehicles that have non-standard wheel configuration (i.e. are not a 4-wheel car).
        ///     Ackerman will be auto-calculated for 4-wheel cars.
        /// </summary>
        [Tooltip(
        ...
        /// </summary>
        [Range(0f, 2f)]
        [ShowInSettings("Handbrake Coefficient", 0f, 2f)]
        ...
        [SerializeField] private List<WheelComponent> wheels = new List<WheelComponent>();
        ...
        public void FindBelongingWheels()
        {
            ...
            if (wheels.Count == 2)
            {
                trackWidth = Vector3.Distance(
                    LeftWheel.wheelController.Visual.transform.position,
                    RightWheel.wheelController.Visual.transform.position);
            }
        }
