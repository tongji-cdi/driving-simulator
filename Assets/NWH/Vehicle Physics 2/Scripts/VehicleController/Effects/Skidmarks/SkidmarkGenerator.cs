...
namespace NWH.VehiclePhysics2.Effects
{
    ...
    public class SkidmarkGenerator
    {
        ...
        public void Update(int surfaceMapIndex, float targetIntensity, Vector3            velocity,        float dt)
        {
            ...
            if (surfaceMapIndex >= 0)
            {
                ...
                Vector3 currentPosition =
                    _currentSkidObject.transform.InverseTransformPoint(
                        _targetWheelComponent.wheelController.wheelHit.groundPoint);
                currentPosition += _targetWheelComponent.wheelController.wheelHit.normal * _skidmarkManager.groundOffset;
        ...
        public void GenerateNewSection()
        {
            ...
            _currentSkidObject = new GameObject($"SkidMesh_{_targetWheelComponent.wheelController.Parent.name}_" +
                                         $"{_targetWheelComponent.wheelController.name}_{_sectionCount}_" +
                                         $"{surfaceName}");
            _currentSkidObject.transform.parent   = _skidmarkManager.skidmarkContainer.transform;
            _currentSkidObject.transform.position = _targetWheelComponent.wheelController.transform.position;
