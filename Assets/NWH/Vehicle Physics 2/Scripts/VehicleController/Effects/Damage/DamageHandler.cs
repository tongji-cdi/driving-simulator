    ...
    public class DamageHandler : VehicleComponent
    {
        ...
        public bool Enqueue(Collision collision, float accelerationMagnitude)
        {
            ...
            if (!visualOnly && damageIntensity > 0)
            {
                ...
                {
                    if (Vector3.Distance(collisionPoint, wc.wheelController.center) <
                        wc.wheelController.wheel.radius * 2f)
                    {
                        wc.Damage += damage;
                    }
                }
        ...
        public void Repair()
        {
            ...
            foreach (PowertrainComponent component in vc.powertrain.Components)
            {
