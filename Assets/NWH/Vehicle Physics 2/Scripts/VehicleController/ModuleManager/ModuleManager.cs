...
namespace NWH.VehiclePhysics2.Modules
{
    ...
    public class ModuleManager : VehicleComponent
    {
        ...
        public void ReloadModulesList(VehicleController vc)
        {
            ...
            if (moduleWrappers.Count == 0 || moduleWrappers == null)
            {
                return;
            }
            ...
            Debug.Assert(modules.Select(m => m.GetType().Name).Distinct().Count() == modules.Count, 
                         "Duplicate module in modules list.");
        }
    }
}